...
Data Layer: MongoDB (NoSQL) for flexible storage of multi-brand sizing schemas.

🚀 Key Engineering Features
📷 Neural Fit Scanner: Extracts body landmarks (Chest, Waist, Hips) via browser-based camera input with A-Pose calibration.

⚖️ Brand Normalization Engine: A custom Java service that translates raw pixel data into standardized metric units (cm).

📊 Fit History Analytics: Persistent tracking of user measurements over time to monitor fitness and styling progress.

⚡ Optimized Performance: Frontend built with Vite and Tailwind CSS for a modern, responsive Dark Mode UI.
...
Database: MongoDB for schema-less brand data flexibility.
...
Icons: Lucide React for minimalist aesthetic.
